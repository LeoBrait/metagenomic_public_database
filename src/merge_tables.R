...
#Creating a list with the file names
#in each folder of the "biome_tables_organized" folder.#
#for this, the function list.files was used,
#with the argument pattern = ".csv" to exlude the readme files.#
dflist <- list.files(path = "data\\03_manual_treatment\\biome_tables_organized",
...
for (item in dflist){
    for (i in 1:n_of_datasets){
        df_[i] <- read.csv(
            file = "data\\03_manual_treatment\\biome_tables_organized\\", item)
    }
}
